...
semantic/dark.json, component.json) and emits:
  ...
  - ../docs/bella.css Copy of bella.css for the GH Pages site
  - ../docs/tokens.md Token reference generated from the rollup
  - ../.storybook/theme-values.json  Storybook manager theme values
  - ../.storybook/public/favicon.svg Storybook favicon (keycap motif)
...
# v0.1 governance tokens (docs/RULES.md). Fixed-context: they do NOT flip with
# [data-theme="dark"] — they name "text on a dark surface" and "focus ring"
# regardless of page mode, because dark surfaces appear inside light pages.
# Pending a proper home in the token JSON tiers (naming/tiering is a design
# decision — Elleta's call); until then they are emitted here so the generated
# CSS keeps the contract RULES.md mandates.
css_lines += [
    "",
    "  /* ---- v0.1 governance tokens (see docs/RULES.md) ----",
...
# ---------- emit Storybook theme values + favicon ----------
# The branded Storybook manager (.storybook/theme.ts) reads these values
# instead of carrying its own copies of any hex. Both files are generated
# artifacts, diff-checked by the gate like bella.css/bella.json.

def tok(path: str) -> str:
    return str(resolve_light(prim_flat[path][0]))

theme_values = {
    "$note": "Generated by tokens/build.py from the token sources. Do not edit by hand.",
    "ground":     tok("color.brand.ground"),
    "ink":        tok("color.brand.ink"),
    "night":      tok("color.night.ground"),
    "iris":       tok("color.brand.iris"),
    "periwinkle": tok("color.brand.periwinkle"),
    "irisDeep":   tok("color.iris.deep"),
    "paper":      tok("color.neutral.paper"),
    "surface":    tok("color.neutral.surface"),
    "border":     tok("color.neutral.border"),
    "inkSoft":    tok("color.neutral.ink-soft"),
    "inkMuted":   tok("color.neutral.ink-muted"),
    "nightCard":  tok("color.night.card"),
    "navyInk":    tok("color.navy.ink"),
    "navyInkMuted": tok("color.navy.ink-muted"),
    "nightDivider": tok("color.night.divider"),
...
SB_DIR = ROOT.parent / ".storybook"
(SB_DIR / "theme-values.json").write_text(json.dumps(theme_values, indent=2) + "\n")

# Favicon: the keycap motif — iris plate, lighter inner plate, ground "B".
# Generated so the hex values track the tokens.
favicon = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
  <!-- Generated by tokens/build.py. Do not edit by hand. -->
  <rect width="32" height="32" rx="7" fill="{tok('color.brand.iris')}"/>
  <rect x="3" y="2" width="26" height="26" rx="6" fill="{tok('color.iris.hi')}"/>
  <text x="16" y="21.5" text-anchor="middle" font-family="Geist, system-ui, sans-serif"
    font-size="17" font-weight="700" fill="{tok('color.brand.ground')}">B</text>
</svg>
"""
(SB_DIR / "public").mkdir(exist_ok=True)
(SB_DIR / "public" / "favicon.svg").write_text(favicon)
...
# ---------- emit docs/bella.css + docs/tokens.md ----------
# docs/ is the GH Pages site. Both files are generated here so they can never
# drift from tokens/ — do not hand-edit them.

DOCS = ROOT.parent / "docs"
(DOCS / "bella.css").write_text((ROOT / "bella.css").read_text())

def md_table(flat, resolver):
    lines = ["| Token | Value | Type |", "|---|---|---|"]
    for path, (v, t, _) in flat.items():
        if isinstance(v, dict):
            continue
        raw = f" (`{v}`)" if isinstance(v, str) and REF.search(v) else ""
        lines.append(f"| `{path}` | `{resolver(v)}`{raw} | {t or ''} |")
    return "\n".join(lines)

tokens_md = f"""# BELLA token reference
...
{md_table(prim_flat, resolve_light)}

## Tier 2 — Semantic (light)

{md_table(light_flat, resolve_light)}

## Tier 2 — Semantic (dark overrides)

{md_table(dark_flat, resolve_dark)}

## Tier 3 — Component

{md_table(comp_flat, resolve_light)}
"""

(DOCS / "tokens.md").write_text(tokens_md)
...
print(f"Wrote docs/bella.css, docs/tokens.md")
print(f"Wrote .storybook/theme-values.json, .storybook/public/favicon.svg")
print(f"")
