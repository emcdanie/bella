...
/* Sidebar lockup: the ink BELLA wordmark (brand refresh, 2026-09-22;
 * generated by scripts/build-favicons.mjs) over the version, read from
 * package.json so it never goes stale. */
const lockup = (_ink: string, muted: string) =>
  `<span style="display:flex;flex-direction:column;gap:6px">` +
  `<img src="./brand/bella-wordmark-ink.svg" alt="BELLA" height="32" style="display:block;height:32px;width:auto" />` +
  `<span style="display:block;font-family:${values.fontMono};font-size:11px;color:${muted}">design system · v${pkg.version.split('.').slice(0, 2).join('.')}</span></span>`;
