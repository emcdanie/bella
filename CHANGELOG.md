...
### Added
- **Motion ramp, two missing steps** (2026-09-20): `motion.duration.slower` (600ms) for entrances such as reveal-on-scroll, and `motion.easing.in-out` (`cubic-bezier(.65,0,.35,1)`) for movement that reverses, so open and close feel the same. The elleta.design portfolio had been carrying both as local copies because BELLA had no entrance step and no symmetric ease; it now aliases BELLA instead. Not breaking: nothing renamed or removed. A third token, `duration.base` (300ms), was proposed and dropped rather than shipped one step from `duration.normal` (250ms); the portfolio retuned to `normal`
